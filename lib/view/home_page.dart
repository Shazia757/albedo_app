...
class HomeView extends StatelessWidget {
  ...
  Widget _studentDashboard(BuildContext context) {
    return Column(
      children: [
        _packagesAnalyticsCard(context),
        const SizedBox(height: 14),
        _youtubeCard(context),
        const SizedBox(height: 14),
        _nextSessionCard(context),
        const SizedBox(height: 14),
  ...
  Widget _studentsAnalyticsCard(BuildContext context) {
    final cs = Theme.of(context).colorScheme;
    final auth = Get.find<AuthController>();
    final role = auth.activeUser?.role;
              ...
              child: LineChart(
                LineChartData(
                  ...
                  lineBarsData: isAdvisor
                      ? [
                          _line(c.totalPackageData, cs.primary),
                          _line(c.totalSpotData, cs.secondary),
                        ]
                      : [
                          _line(c.totalSalary, cs.primary),
                          _line(c.pendingSalaryData, cs.secondary),
                          _line(c.receivedSalaryData, cs.tertiary),
                          _line(c.totalHoursData, Colors.green),
                        ],
                ),
              ),
            ),
  ...
  Widget _packagesAnalyticsCard(BuildContext context) {
    final cs = Theme.of(context).colorScheme;
      ...
      return Container(
        ...
        child: Column(
          ...
          children: [
            ...
            Row(
              children: [
                _legendDot(context, cs.primary, "Total Fee"),
                const SizedBox(width: 10),
                _legendDot(context, cs.secondary, "Pending Fee"),
                const SizedBox(width: 10),
                _legendDot(context, cs.tertiary, "Total Classes"),
                const SizedBox(width: 10),
                _legendDot(context, Colors.green, "Hours"),
              ],
            ),
            ...
            Expanded(
              child: LineChart(
                LineChartData(
                  ...
                  borderData: FlBorderData(show: false),
                  lineBarsData: [
                    _line(c.totalFeeData, cs.primary),
                    _line(c.pendingFeeData, cs.secondary),
                    _line(c.totalClassesData, cs.tertiary),
                    _line(c.totalHoursData, Colors.green),
                  ],
                ),
              ),
  ...
  Widget _hiringSection(BuildContext context) {
    return Obx(() {
    ...
    return SizedBox(
      ...
      child: PageView.builder(
        ...
        itemBuilder: (context, index) {
          return _hiringCard(context, data[index]);
        },
      ),
    );
  }

  Widget _hiringCard(BuildContext context, HiringView item) {
    final cs = Theme.of(context).colorScheme;
    final ad = item.ad;
              ...
              children: [
                ...
                if (ad.days != null && ad.days!.isNotEmpty)
                  Wrap(
                    spacing: 6,
                    children:
                        ad.days!.map((d) => _dayChip(context, d.name)).toList(),
                  ),
      ...
      return Container(
        ...
        child: Column(
          ...
          children: [
            ...
            /// 🔹 CAROUSEL
            _recommendationCarousel(context, c.recommendations),
          ],
        ),
      );
    });
  ...
  Widget _recommendationCarousel(
    ...
    return SizedBox(
      ...
      child: PageView.builder(
        ...
        itemCount: data.length,
        itemBuilder: (context, index) {
          return _recommendationCard(context, data[index]);
        },
      ),
    );
  }

  Widget _recommendationCard(BuildContext context, RecommendationView item) {
    final cs = Theme.of(context).colorScheme;
  ...
  Widget _dayChip(BuildContext context, String day) {
    final cs = Theme.of(context).colorScheme;
  ...
  Widget _nextSessionCard(BuildContext context) {
    final auth = Get.find<AuthController>();
    final role = auth.activeUser?.role;
      ...
      return Container(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 12),
            _sessionMainCard(context, session, isTeacher),
            ...
            Wrap(
              ...
              runSpacing: 8,
              children: [
                _infoCard(context, "Subject", session.package ?? "-"),
                _infoCard(context, "Syllabus", session.syllabus ?? "-"),
                _infoCard(context, "Standard", session.className ?? "-"),
              ],
            ),
            ...
            child: Column(
              ...
              children: [
                ...
                Container(
                  ...
                  child: Text(
                    isStarted ? "Started" : _startsInText(session),
                    style: context.textTheme.labelSmall?.copyWith(
  ...
  String _startsInText(Session session) {
